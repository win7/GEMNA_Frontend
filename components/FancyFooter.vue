<template>
	<footer id="sc-fancy-footer">
		<nav>
			...
			<ul id="fancy-footer-switcher" ref="list" class="sc-ff-list">
				<li
...
export default {
		...
		'listItems': {
			...
			handler (items) {
				...
				if(index > -1) {
					...
					UIkit.switcher('#fancy-footer-switcher').show(index)
				}
			}
		}
	},
	mounted () {
		this.$nextTick(() => {
			...
			if(index) {
				...
				this.activeIndex = index
				// init switcher
				UIkit.switcher('#fancy-footer-switcher', {
					connect: '#fancy-footer-switcher-items',
	...
	methods: {
		...
		alignWave (index) {
			...
			this.waveStyle.left = `${itemPosLeft.toFixed(2) - itemWidth/2 - 7}px`
		}
	}
}
</script>
